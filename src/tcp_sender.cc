...
void TCPSender::push( const TransmitFunction& transmit )
{
  ...
  while(sequence_numbers_in_flight()<win_siz){
    ...
    if(input_.has_error()) res.RST=1;
    uint64_t sent_tmp=sent;
    if(!syn_set) syn_set=1,res.SYN=1,outgoing++,sent++;
    ...
    if(!fin_set&&(writer().is_closed())&&outgoing<win_siz&&tmp1.size()==0) fin_set=1,res.FIN=1,outgoing++,sent++;
    if(!res.sequence_length()) break;
    transmit(res);
    if(!intime) intime=1,total_time=0;
    int flag=0;
    for(size_t i=0;i<inflight.size();i++){
    ...
    if(!flag) inflight.push_back(res);
    if(tmp1.size()==0||reader().is_finished()) break;
  }
}
...
void TCPSender::receive( const TCPReceiverMessage& msg )
{
    ...
    if(inflight.size()>0){
      ...
      if(val3<val1){
        ...
        if(!flag){
          ...
          if(val2<val1&&(inflight[inflight.size()-1].sequence_length()+val2==val1)) inflight.clear();
  ...
  if(tmp1!=outgoing){
    ...
    retransmission=0;
    if(outgoing!=0) total_time=0;
  }
  if(!inflight.size()) intime=0,total_time=0;
}
