...
{
  // Your code here.
  if(is_last_substring) eof_index=first_index+data.size();
  // first slice
  if(first_index<expected){
    if(expected-first_index>data.size()) return;
  ...
  if(first_index-expected>output_.writer().available_capacity()) return;
  ...
  pair<uint64_t,string> now;now.first=first_index,now.second=data;
  int flag=0;
  for(size_t i=0;i<buffer.size();i++){
    uint64_t l=buffer[i].first,r=buffer[i].first+buffer[i].second.size()-1;
    if(now.first>r) continue;
    else if(now.first>=l){
  ...
  for(size_t i=0;i<buffer.size()-1;i++){
    if((buffer[i].first>buffer[i+1].first)&&buffer.size()==3) assert(0);
  }
  for(size_t i=0;i<buffer.size();i++){
  ...
  while(buffer.size()>0&&buffer[0].first<=expected) buffer.erase(buffer.begin());
  if(eof_index!=1145141919810&&eof_index==expected) output_.writer().close();
}
